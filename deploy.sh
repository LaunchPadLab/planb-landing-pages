...
if [ -e "$DEPLOYMENT_TARGET/Gemfile" ]; then
  ...
  # maybe can do if statements for not resetting ruby version if already have it
  echo "Setting ruby version"
  rbenv global $WEBSITES_LATEST_NODE_VERSION
  exitWithMessageOnError "Failed to switch ruby versions"

  # maybe can do if statements for not installing bundler if already installed
  echo "Installing bundler version 1.16.0"
  gem install bundler -v 1.16.0
  bundle config --local path "vendor/bundle"
  # maybe don't need bundle clean
